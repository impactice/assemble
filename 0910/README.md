...
.data
    ; 발표할 때 설명하기 좋은 변수 선언
    num1 DWORD 20      ; 20이라는 숫자를 num1에 저장
    num2 DWORD 30      ; 30이라는 숫자를 num2에 저장
    result DWORD 0     ; 결과값을 저장할 공간 (처음엔 0)

.code
main PROC
    ; 1. 레지스터에 값을 넣고 더하는 과정 수행
    mov eax, num1      ; EAX 레지스터에 num1의 값(20)을 옮김
    add eax, num2      ; EAX 레지스터에 num2의 값(30)을 더함 (EAX = 50이 됨)
    
    ; 2. 계산된 결과를 변수에 저장
    mov result, eax    ; result 변수에 최종 계산된 50을 저장
